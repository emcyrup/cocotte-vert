...
毎日、設定した時刻（既定 10:00 JST。リマインドごとに 9〜20 時で変更可）に以下を実行する。実行結果は通数を消費しないよう Push せず保存し、
スタッフ用 LINE グループで **「配信結果」** と発言すると応答メッセージで返す（応答は無料）。
同じグループで **「今日の予約」「明日の予約」「8/20の予約」** と聞くと、その日の一覧が返る
（読み取り専用・通数無料。対象グループ以外では反応しない）。
...
EPARK からも予約が入り続けるので、こちらで入った予約の時間は EPARK 側の枠も閉じる必要がある。
EPARK に API は無いため、**管理画面をブラウザ操作して仮受付で埋める**。
予約が入った・変わった・取り消された時点で走り（`EPARK_TRIGGER`）、取りこぼしは30分ごとの
定期実行が拾う。既定は `EPARK_MODE=off`（一切触らない）。
設計と実物の画面の癖は [docs/import-api.md](docs/import-api.md#epark-管理画面の自動操作rpa) に集約。
...
配信時刻も `/mock/#rem` でリマインドごとに選べる（9〜20時の毎時、既定 10:00）。
同じ画面の「追加リマインド」で、**スタッフ独自の配信ルール**（来店から◯日後／予約の◯日前＋
自由な文面）も作れる。詳細は [docs/spec.md](docs/spec.md) の 2-4-2。

## 店舗管理画面

`/mock/` がスタッフ向けの店舗管理画面（Basic 認証の内側）。管理 API に届く本番環境では
**実データで動き**、単体で開いたときは提案・要件確認用のデモとして動く。
予約管理・ペットカルテ・予約カレンダー・週次シフト表・シフト申請・SNS投稿は実装済み。
予約カレンダー上のドラッグでのコピー・時間変更だけは画面上の操作にとどまり、保存されない。
