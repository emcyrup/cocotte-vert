...
| 自動デプロイ（`deploy-prod`） | **完了**。SSH → clone → `npm ci` → 起動まで通っている |
...
| **EPARK 側の枠を閉じる** | **稼働中**（2026-08）。EPARK の協力は得られず、ブラウザ操作による自動化に切り替えた。GitHub Actions（`epark-sync`）で**閉じる／開け直す**を `live` で回しており、実物で往復を確認済み。予約が入った・直した・取り消した時点で**即時反映**（`EPARK_TRIGGER`）、取りこぼしは**30分ごとの定期実行**が拾う。仮受付には**院内メモでお名前・電話番号・コース・予約番号を載せる**（EPARK 側の顧客台帳には登録しない）。失敗は予約管理の「EPARK未反映」カードに残り Slack へ飛ぶ。EPARK の予約は店舗の承認制なので、閉じ忘れても二重予約は起きない（断りの手間が増えるだけ）。詳細は `docs/import-api.md` |
