...
| **EPARK 側の枠を閉じる** | **チェックリストは実装済み**（予約管理の「EPARK未反映」カード）。EPARK の協力は得られず（2026-08）、**ブラウザ操作による自動化が進行中**。骨組み（`EPARK_MODE`・読み直し確認・失敗を画面に残す）と駆動部は実装済みで、実物での点検も通っている。仮受付には**お名前・電話番号・コースを院内メモで載せる**（EPARK 側の顧客台帳には登録しない）。残るのは実物での `dry_run` → `live`。EPARK の予約は店舗の承認制なので、閉じ忘れても二重予約は起きない（断りの手間が増えるだけ） |
