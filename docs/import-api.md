...
**⑪ 確認画面には「はい」で答える。** 「仮受付」には確認画面が出ないが、
**「キャンセル」には出る**（実物で確認）。自動操作は既定で確認画面に「いいえ」を
答えるため、押しても何も起きないまま待ち時間切れになる。こちらが起こした操作の
確認なので受け入れる。**押せたかどうかは④のとおり画面を読み直して確かめる**ので、
受け入れたせいで勝手に進むことはない。確認画面の文面には氏名が混じりうるので、
ログには種類（`confirm` など）だけを残す。

> 開け直せなかったときは、**枠の受付番号が変わっていないか**を見て
> 「押しても何も起きていません」と書き添える。確認画面を越えられていないのか、
