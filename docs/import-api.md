...
> もともとは仮受付の印（`li.tentative-reservation`）で見分けていた。ところが**顧客情報に
> お名前を入れると、その枠には印が付かなくなる**（実物で確認）。受付番号なら1件ごとに違うので、
> スタッフが手作業で入れた仮受付とも区別できる。**印より確実。**
...
**顧客情報の欄**（`…TxtLastName` / `…TxtFirstName` / `…TxtTel`）。ここに入れると
**受付表の一覧にそのまま名前が出る**。かな・カード番号はこちらが持っていないので空欄。
氏名は1つの欄で持っているため、最初の空白で姓と名に割る（空白が無ければ全部を姓に入れる）。

**院内メモ**（`#txtMemoNow`）。開いたときに見える1行:
