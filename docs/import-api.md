...
### 仮受付にお名前を載せる（2026-08 追加）

無名の仮受付で枠を押さえるだけでは、EPARK の受付表を見ても**誰のご予約か分からない**。
結局こちらの画面と突き合わせる手間が残る。そこで、空き枠の「受付」から開く
**「顧客検索及び新規受付登録」**の画面を使い、院内メモにお名前を書いてから仮受付にする。
...
末尾の `res=12` はこちらの予約番号。EPARK 側で見つけた受付を、こちらの画面ですぐ引ける。

**ここで踏み込まないことを4つ決めている。**

**⑥ EPARK 側の顧客台帳は検索も選択も新規登録もしない。** 登録画面には電話番号での顧客検索が
付いていて、選べば正式な「受付」にできる。だが同姓同名や複数ヒットで**別のお客様に
紐づけると、相手の台帳が壊れて元に戻せない**。院内メモに書くだけなら、間違っていても消せる。

**⑦ 押すのは「受付」(`#OP0062UD01`) ではなく「仮受付」(`#OP0062UD02`)。** 正式な受付にすると
`li.tentative-reservation` の印が消え、**本物のご予約と見分けが付かなくなる**。そうなると
取消のとき自分が入れた分を開け直せない（③⑤の守りが丸ごと効かなくなる）。
なお「受付」は顧客を選ぶまで `disabled` で、「仮受付」は選ばなくても押せる。

**⑧ コース (`#reservationIdName`) は選ばない。** 選ぶと `scheduleEndtimeChange()` が終了時刻を
勝手に動かし、閉じる枠がこちらの所要時間とずれる。枠の範囲は②のとおりこちらで決める。
受付通知メモ (`#txtMemoNotice`) も使わない（お客様側へ出る可能性がある）。

**⑨ 開いた登録画面が本当にその枠のものか確かめてから押す。** 画面は日付・時刻・ラインを
hidden で持っている（`hidSearchCustomerAndRegisterAppoint…`）。これが閉じるつもりの枠と
一致しなければ**何も書かずに引き返す**。取り違えたまま押すと、まったく違う時間に
ご予約を入れてしまう。

登録画面は1枠ずつしか開けないので、**お名前が載るのは先頭の枠だけ**。90分の予約の2枠目は
これまでどおり一括の仮受付で押さえる。

落ちどころも決めてある。**登録画面が開かない・確認が通らない・押せないときは、
これまでどおりの無名の仮受付に落として枠だけ押さえる。** 相手の画面が変わっても、
最悪でも今の動きに戻るだけで、枠を取り逃がすことはない。

`EPARK_DETAILS=off`（または `config/epark-profile.json` の `register` を消す）で
無名の仮受付に戻せる。

> **個人情報について。** 自動化は GitHub Actions で動き、作業の一覧は管理 API から取る。
> 既定の `?fields=sync` には氏名・電話番号が入らない。お名前を載せる運用のときだけ
> `&details=1` を付けて必要な分だけ足す。**取ってきた文字列はログにも Slack にも出さない**
> （画面に打ち込むだけ）。開け直すだけの予約には氏名を渡さない。
...
| `src/epark/details.js` | 仮受付に添える「誰のご予約か」の1行。**ログには出さない** |
