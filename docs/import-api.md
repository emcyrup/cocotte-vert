...
状態は続く。こちらで入った予約の時間帯を EPARK 側で閉じておかないと、同じ枠に別のお客様が
入りダブルブッキングになる。

**EPARK には外部から予約枠を操作する口が無いため、閉じる作業は手作業。**
...
EPARK 側に次のいずれかがあれば、この手作業は要らなくなる。**確認待ち**。

- 外部からの予約登録 API（こちらの予約を EPARK へ書き込む）
- iCal 購読（こちらの予定表を EPARK に読ませる）
- 予約枠の一括休止（時間帯をまとめて閉じる機能）
