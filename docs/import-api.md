...
**1. 3段階のガード（`EPARK_MODE`）。** 既定は `off`。明示しない限り EPARK を一切触らない。
...
**2. 書いたら必ず読み直す。** `driver.isSlotClosed(slot)` で画面から読み直し、意図どおりに
なっていたときだけ済みにする。読み直せない駆動部は `isValidDriver` で弾き、実行させない。

**3. 失敗はチェックリストに残す。** 済みにしないので画面の「EPARK未反映」に残り、
...
| チェックボックス | `<label><input type="checkbox"><span class="box"></span></label>`。**`input` は CSS で隠れている**ので `label` を押す |
...
**③ 開け直すのは自分が入れた枠だけ。** 見分けには **EPARK の受付番号**を使う。閉じたときに
枠が持っている `multiSchedulerHidAppointId` を控えておき、開け直すときに一致する枠だけを
取り消す。番号が違えば、こちらの受付は消されていて別の受付が入っている＝**絶対に触らない**
（こちらの取消と入れ違いで EPARK からご予約が入っていることがある）。

> もともとは仮受付の印（`li.tentative-reservation`）だけで見分けていた。お名前を一覧に
> 出す試みは取りやめたので（後述）、こちらの枠には印が付く。それでも**受付番号を主に据える**:
> 番号は1件ごとに違うので、スタッフが手作業で入れた仮受付とも区別できる。**印より確実。**
> 番号を控えていない枠（この仕組みより前に閉じたもの）は、従来どおり印で見分ける。

**④ 押したあと必ず読み直す。** 「仮受付」を押しても**確認画面が出ない**ので、
押せたかどうかを教えてくれるものが何も無い。受付表の手順の最後に `waitFor: {closed}` を置いて
**その枠が実際に変わるまで待ち**、さらに画面を開き直して読み直す。変わっていなければ
「閉じられませんでした」として失敗させ、チェックリストに残す。

> **登録画面の手順には置かない。** 実物は登録しても**受付表へ戻らない**ので、
> 枠のセレクタを待つと入っているのに毎回15秒待って紛らわしい警告が出る（実物で踏んだ）。
> 押したあとは画面が落ち着くまでだけ待ち（送信の途中で打ち切らないため）、
> **入ったかどうかは受付表を開き直して読む**。判断の場所は1つに寄せる。

> **手順が転んでも読み直す。** 実物では「仮受付は入ったのに、入ったことを待ち構える
> ところが時間切れ」という転び方をした。押す前にその枠が「開」だと確かめてあるので、
> あとで「閉」になっていれば自分が閉じたもの。**判断は例外の有無ではなく、画面の状態で行う。**
> 書けていたのに失敗として残すと、チェックリストに「もう閉じている枠」が並び続ける。

**⑤ キャンセルの確認は「画面の中の小窓」。** 「仮受付」には確認が出ないが、
**「キャンセル」には出る**（実物で確認）。ただしこれはブラウザの確認画面ではなく、
**ページの中に描かれた小窓**（`確認 / 受付をキャンセルします。よろしいですか？ /
OK・キャンセル`）。つまり自動操作の「確認画面に答える」仕掛けでは越えられず、
**OK を自分で押しに行く**しかない。手順に1つ足してある。

```json
{ "click": "button:visible:text-is(\"OK\"), input[type=\"button\"][value=\"OK\"]:visible, a:visible:text-is(\"OK\")" }
```

実物の小窓が `button` なのか `input` なのか分からないので広めに取り、`:visible` で
**出ている小窓のもの**だけに絞っている。実物の HTML が分かればここを絞ること。

> ブラウザの確認画面が出る作りに変わっても越えられるよう、そちらにも「はい」で
> 答えるようにしてある（保険）。文面には氏名が混じりうるので、ログには種類
> （`confirm` など）だけを残す。**どちらの場合も、押せたかどうかは④のとおり
> 画面を読み直して確かめる**ので、答えたせいで勝手に進むことはない。

> 開け直せなかったときは、**枠の受付番号が変わっていないか**を見て
> 「押しても何も起きていません」と書き添える。小窓を越えられていないのか、
> 押す場所そのものが違うのかを、ログだけで切り分けるための手がかり。

**⑥ 営業時間をはみ出した枠は飛ばす。** 予約は営業時間の終わりをはみ出す。実物の
トリミングは **17:00 が最後の枠**なのに 17:00-18:30 の予約が来て、18:00 を触りにいって
**毎回失敗し続けていた**（予約が入るたびに失敗が増える）。はみ出したぶんは相手に枠が
無いだけなので、そこは飛ばして残りを進める。飛ばしたことはログに残す。

> **1つも読めないなら飛ばさない。** 設定が実物と合っていない・日付が違う、という
> 壊れ方と区別が付かなくなり、**何もしていないのに済みにしてしまう**ため。
> 「一部だけ読めない＝はみ出した」「全部読めない＝壊れている」で分ける。
...
**⑧ 他のご予約が使っている枠は開けない。** 予約は重なる。取消の 13:30-14:30 を開け直すとき、
14:00 の枠を別の確定予約（14:00-15:00）が使っていることがある。そこを開けると**その
ご予約のぶんの枠まで空いてしまう**。しかもその予約は「反映済み」のままなのでチェックリストにも
出ず、**誰も気付けない**。一覧に「同じ日の他の確定予約」を添えて渡し、その枠は閉じたまま残す。
ラインが違えば別の列なので関係ないが、**どちらかのラインを決められないときは触らない側に倒す**
（開けて取り逃がすより、閉じたまま残すほうが軽い）。開ける枠が1つも残らなければ、
EPARK を見に行かずに済みにする（残すとチェックリストに居座り続ける）。

### 仮受付にお名前を載せる（2026-08 追加）

無名の仮受付で枠を押さえるだけでは、EPARK の受付表を見ても**誰のご予約か分からない**。
結局こちらの画面と突き合わせる手間が残る。そこで、空き枠の「受付」から開く
**「顧客検索及び新規受付登録」**の画面を使い、2か所に書いてから仮受付にする。

**顧客情報の欄には入れない**（`…TxtLastName` / `…TxtFirstName` / `…TxtTel`）。
一度は入れていた。一覧にそのまま名前が出るので良さそうに見えたが、**実物では
お客様の欄を埋めると「仮受付」ではなくなり、ボタンが `disabled` になる**。
押せないまま終わるので、**名前も院内メモも入らずに無名の仮受付へ落ちていた**。
埋めないことが、この画面を使える唯一の道。

> 実物での確かめ方: 何も入力しなければ仮受付できる。受付内容（コース）を選ぶと
> 仮受付が消える。**お客様の欄も同じ扱い**で、埋めた時点で仮受付ではなくなる。
> 名前を一覧に出したいなら「受付」を押すしかなく、それは⑩のとおり取り消せない。

**院内メモ**（`#txtMemoNow`）だけを使う。開いたときに見える1行:
...
付いていて、選べば台帳の顧客に紐づく。だが同姓同名や複数ヒットで**別のお客様に
紐づけると、相手の台帳が壊れて元に戻せない**。打ち込むだけなら、間違っていても消せる。

**⑩ 押すのは「受付」(`#OP0062UD01`) ではなく「仮受付」(`#OP0062UD02`)。** 正式な受付にすると
**取り消せなくなる**。なお「受付」は顧客を選ぶかお名前を入れるまで `disabled`。

> **お名前を入れると仮受付の印が消える**（実物で確認）。もともとはこの印で自分の枠を
> 見分けていたので、そのままでは取消のときに開け直せなくなる。だから③の見分けを
> **受付番号**に変えた。番号は1件ごとに違うので、印より確実に自分の枠だと分かる。
...
- `register.steps` の `value` でだけ、お客様の情報を差し込める:
  `{lastName}` `{firstName}` `{phone}` `{details}`。**セレクタ側には使わせない**（検証で弾く）
- `cell.appointId` は任意だが、無いと「お名前を載せた枠」を自分のものと見分けられない
...
| `register` の開くところにお客様の情報 | セレクタに氏名が混ざると壊れる |
...
定期実行は30分ごと。それでは「予約を入れたのに EPARK がまだ空いている」時間が最大30分残る。
そこで**予約が入った・変わった・取り消された時点で、アプリが上のワークフローを起こす**。

**アプリから EPARK を直接触ることはできない。** 本番サーバーには sudo が無く Chromium を
置けないため、ブラウザがあるのは GitHub Actions だけ。ここで「即時」にできるのは
**向こうを今すぐ起こすこと**までで、起動から反映までは1〜2分かかる（ランナーの用意と
ブラウザの導入）。真に瞬時にするにはアプリの隣にブラウザが要る＝別のサーバーが要る。

| `.env` | 中身 |
| --- | --- |
| `EPARK_TRIGGER` | `off`（既定）/ `on`。3つ揃わないまま `on` にすると起動時に落ちる。
大文字や `true` / `1` / `yes` も有効として読み、**どちらとも読めない値は起動時に断る**
（書き方の揺れで黙って off に落ちるのが一番たちが悪い） |
| `GITHUB_REPO` | `owner/repo` の形 |
| `GITHUB_ACTIONS_TOKEN` | **Actions への書き込み権限だけ**の細粒度 PAT |

**モードは渡さない。** 起こすだけで、`live` か `dry_run` かは向こうの `EPARK_SYNC_MODE` が
持つ。**実際に書き換える判断を1か所にまとめる**ため（送信の `SEND_MODE` と同じ考え方）。

**効いているかは `/health` で分かる。** `{"eparkTrigger":"on"}` が出ていなければ、
`.env` が読めていないか再起動されていない。起動ログにも1行出す。
**黙って off になっていて外から分からない**、という状態を作らないため。

**起こせなくても予約は通る。** 通信が落ちても認証が切れても、予約の登録は成功として返し、
Slack に「起こせませんでした／定期実行が拾います」と出す。**反映が遅れるだけで止まらない。**

**続けて予約が入っても走るのは1本。** アプリ側で「走っている間の分は終わってから1回だけ」に
まとめ、ワークフロー側も `concurrency: epark-sync` で待たせる。一覧は毎回まるごと消化するので、
待ちが1つあれば取りこぼさない。**同じ受付表を2つのブラウザで同時に触らせない**ことが要。

> 来店・無断キャンセルでは起こさない。枠はもう閉じてあり、当日の枠を今から開けても意味がない。
