...
- [x] 店舗管理画面「シフト入力」への組み込み

**スタッフの識別**: LINE の userId は本人からは見えないため、管理画面で6桁の合言葉を発行し、
本人が「スタッフ登録 123456」と送って紐付ける（有効期限24時間・使い捨て）。
...
**未対応（構想のまま）**: 週次シフト表そのもののデータ化。現状は承認済みの申請が一覧に
残るだけで、シフト表（`/mock/#shift` 上部）はサンプル表示のまま。
