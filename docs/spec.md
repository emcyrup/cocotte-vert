...
**本文は管理画面から上書きできる**（`app_settings.reminder_texts`、500文字まで）。
書き換えられるのは各メッセージの本文だけで、宛名・予約の詳細・ボタン・配信停止導線は
構造として固定。空にして保存すると上書きが消え、コードの既定文（テストで固定している側）に
戻る。テスト送信も上書き後の文面を使うので、書き換えたら実物で確かめられる。
