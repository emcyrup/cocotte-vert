...
// ---- 顧客管理（参照・編集） ----
async function loadCustomerAdmin() {
  const q = document.getElementById('cust-admin-search').value.trim();
  const { customers } = await api(`/customers?q=${encodeURIComponent(q)}`);
  const tbody = document.getElementById('customer-rows');
  tbody.innerHTML = '';
  for (const c of customers) tbody.appendChild(customerRow(c));
  if (customers.length === 0) {
    const tr = document.createElement('tr');
    const td = document.createElement('td');
    td.colSpan = 7;
    td.className = 'empty';
    td.textContent = '該当する顧客がいません';
    tr.appendChild(td);
    tbody.appendChild(tr);
  }
}
...
function customerRow(c) {
  ...
  const td = (label, text, nowrap = false) => {
    const el = document.createElement('td');
    el.dataset.label = label;
    el.textContent = text;
    if (nowrap) el.className = 'nowrap';
    return el;
  };

  const view = () => {
    tr.innerHTML = '';
    tr.appendChild(td('氏名', c.name));
    tr.appendChild(td('電話番号', c.phone_norm ?? '-', true));
    tr.appendChild(td('誕生日', c.birthday ? c.birthday.slice(0, 10) : '-', true));
    tr.appendChild(
      td('最終来店', c.last_visit_at ? fmtJst(c.last_visit_at).replace(/\s.*$/, '') : '-', true)
    );
    tr.appendChild(td('LINE', c.line_linked ? '連携済' : '-', true));
    tr.appendChild(td('配信', c.opt_out ? '停止中' : c.is_blocked ? 'ブロック' : '○', true));
  ...
  const edit = () => {
    tr.innerHTML = '';
    const nameIn = Object.assign(document.createElement('input'), { value: c.name });
    const phoneIn = Object.assign(document.createElement('input'), {
      type: 'tel',
      value: c.phone_norm ?? '',
      placeholder: '電話番号',
    });
    const bdayIn = Object.assign(document.createElement('input'), {
      type: 'date',
      value: c.birthday ? c.birthday.slice(0, 10) : '',
    });
    for (const [label, input] of [['氏名', nameIn], ['電話番号', phoneIn], ['誕生日', bdayIn]]) {
      const el = document.createElement('td');
      el.dataset.label = label;
      el.appendChild(input);
      tr.appendChild(el);
    }
    tr.appendChild(td('最終来店', c.last_visit_at ? fmtJst(c.last_visit_at).replace(/\s.*$/, '') : '-', true));
    tr.appendChild(td('LINE', c.line_linked ? '連携済' : '-', true));

    const optTd = document.createElement('td');
    optTd.dataset.label = '配信';
    const optLabel = document.createElement('label');
    const optIn = Object.assign(document.createElement('input'), {
      type: 'checkbox',
      checked: c.opt_out,
    });
    optLabel.append(optIn, ' 停止');
    optTd.appendChild(optLabel);
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const messages = {
          phone_exists: 'この電話番号は別の顧客に登録済みです',
    ...
    cancelBtn.addEventListener('click', view);
...
document.getElementById('cust-admin-form').addEventListener('submit', (e) => {
  e.preventDefault();
  loadCustomerAdmin().catch((err) => showMsg(`エラー: ${err.message}`));
});
loadCustomerAdmin().catch((err) => showMsg(`エラー: ${err.message}`));
