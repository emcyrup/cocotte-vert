// 予約時刻を過ぎた Instagram 投稿を公開する。数分おきの cron から呼ばれる。
//
...
export function createSnsPublisher({ pool, instagram, slack, config }) {
  ...
  async function publishOne(postId) {
    ...
    try {
      const parts = splitIntoPosts(photos.map((p) => p.file), post.caption);
      const mediaIds = [];
      let dryRun = false;
      ...
      await slack.notify(`:warning: *Instagram 投稿に失敗しました*（post=${postId}）\n${err.message}`);
